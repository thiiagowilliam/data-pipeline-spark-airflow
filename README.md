...
pytest tests/ -v --cov=simulator --cov-report=term-missing
...
| Arquivo | O que cobre |
|---------|-------------|
| `tests/test_data_simulator.py` | `DataGenerator`, `MinioUploader` (mock S3) |
| `tests/test_validator.py` | `DataValidator`: schema, nulos, regras de negócio, `ValidationReport` |
...
|----------|--------|-----------|
| `MINIO_URL` | `http://localhost:9000` | Endpoint MinIO (local) |
| `MINIO_ENDPOINT` | `http://minio:9000` | Endpoint MinIO (Docker) |
| `DB_HOST` | `postgres` | Host PostgreSQL |
| `CLIENT_ROWS` | `550000` | Linhas geradas por ciclo (clientes) |
| `SALES_ROWS` | `700000` | Linhas geradas por ciclo (vendas) |
| `CONTRACT_DIR` | `/opt/airflow/dags/contracts` | Diretório dos contratos JSON |

## Roadmap — Medallion Architecture

A arquitetura atual implementa a **camada Bronze** (raw → bronze). O roadmap prevê:

```
Raw (CSV/MinIO)
    ↓  [validação + ingestão]  ← implementado ✅
Bronze (Delta Lake — dados brutos válidos)
    ↓  [deduplicação + tipo casting + joins]  ← próxima etapa 🔜
Silver (Delta Lake — dados limpos e enriquecidos)
    ↓  [agregações + métricas de negócio]  ← backlog 📋
Gold (PostgreSQL/BI — dados prontos para consumo)
```

| Camada | Status | Descrição |
|--------|--------|-----------|
| **Raw** | ✅ | CSVs no MinIO, gerados pelo simulator |
| **Bronze** | ✅ | Delta Lake com validação por contrato |
| **Silver** | 🔜 | Deduplicação, casting, joins de referência |
| **Gold** | 📋 | Agregações e KPIs para consumo por BI |
